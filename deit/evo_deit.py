...
from timm.data import IMAGENET_DEFAULT_STD
            ...
            for index, blk in enumerate(self.blocks):
                ...
                if index < 4:
                    ...
                    if index == 0:
                        ...
                        cls_attn = (
                            1 - self.tradeoff[index]
                        ) * cls_attn + self.tradeoff[index] * torch.sum(
                            attn[:, :, 0, 1:], dim=1
                        )
                    ...
                    # with torch.no_grad():
                    if self.training:
                        temp_cls_attn = (1 - self.tradeoff[index]) * cls_attn[
                            :, :N_
                        ] + self.tradeoff[index] * torch.sum(attn[:, :, 0, 1:], dim=1)
                        cls_attn = torch.cat((temp_cls_attn, cls_attn[:, N_:]), dim=1)

                    else:
                        cls_attn[:, :N_] = (1 - self.tradeoff[index]) * cls_attn[
                            :, :N_
                        ] + self.tradeoff[index] * torch.sum(attn[:, :, 0, 1:], dim=1)
